"""Tests for the provider-agnostic LLM client (Hugging Face default, Ollama
and Mistral optional).
...
def test_llm_client_defaults_to_huggingface_provider():
    """LLMClient defaults to the configured provider (huggingface) and model."""
    client = LLMClient()
    assert client.provider == "huggingface"
    assert client.model == "mistralai/Mistral-7B-Instruct-v0.2"
